...
class CustomEnv2(gym.Env):
    def __init__(self,grid_size = gridsize[envselector], max_steps=  maxsteps[envselector], goalselector = goalselector):
        ...
        # Load the dataset
        try:
            self.dataset = h5py.File('/home/paperspace/decision-diffuser/code/analysis/newdataset/withoutGoal/my_datasetExperiment2a.hdf5', 'r')#my_datasetExperiment2a
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.obstacle_positions = self._generate_obstacle_positions()
        self.starting_pos, self.goal_position = self.generate_valid_start_goal(self.obstacle_positions)
    ...
    def _calculate_reward1(self, new_position):
        ...
        reward = step_penalty * distance_to_goal

        return step_penalty #reward #step_penalty#reward 
    ...
    def step(self, action):
        ...
        reward = self._calculate_reward1(new_position)
        self.current_step +=1
        ...
        return self._get_observation(), reward, done, {}#self.agent_position
    ...
    def generate_valid_start_goal(self,obstacle_positions):
        possible_starts = [(1,1),(1,4)]
        possible_ends = [(4,4),(4,1)]
        while True:
            #(4,0) #possible_starts[self.goalselector]
            #obstacle_coordinates = [(5,2), (5,6)]
            topdowngen = np.random.randint (0,2)
            
            self.start_position =  possible_starts[topdowngen]
            self.goal_position = possible_ends[topdowngen]
            #if topdowngen ==0:
