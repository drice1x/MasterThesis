...
class CustomEnv2(gym.Env):
    def __init__(self,grid_size = gridsize[envselector], max_steps=  maxsteps[envselector], goalselector = goalselector):
        ...
        # Load the dataset
        try:
            self.dataset = h5py.File('/home/paperspace/decision-diffuser/code/analysis/newdataset/withoutGoal/my_datasetExp2_SmallEval.hdf5', 'r')
    ...
    def generate_valid_start_goal(self,obstacle_positions):
        #possible_starts = [(np.random.randint (1,2),np.random.randint (0,2)),(np.random.randint (1,2),np.random.randint (4,6))]
