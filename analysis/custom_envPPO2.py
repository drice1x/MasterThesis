...
envselector = 1 #np.random.randint(0, 3)# choose which grid is displayed
goalselector = np.random.randint(0,2)

class CustomEnv2(gymm.Env):
    def __init__(self,grid_size = gridsize[envselector], max_steps=  maxsteps[envselector], goalselector = goalselector):
        super().__init__()

        # Load the dataset
        #try:
        #    self.dataset = h5py.File('/home/paperspace/decision-diffuser/code/analysis/my_dataset.hdf5', 'r')
        ...
        self.starting_pos = (0,0) #(np.random.randint(0, 7), np.random.randint(0, 7)) #completely random start position
    ...
    def normalize_observation(self, observation):
        min_value = 0
        max_value = 30  # Change the max_value to 1 for 0-1 normalization
    ...
    def generate_valid_start_goal(self,obstacle_positions):
        possible_starts = [(1,4),(1,1)] # original standard [(1,4),(1,1)]
        possible_ends = [(4,1),(4,4)]
        while True:
            #(4,0) #possible_starts[self.goalselector]
            #obstacle_coordinates = [(5,2), (5,6)]
            topdowngen = np.random.randint (0,2)
            
            start_position =  possible_starts[topdowngen]
            goal_position = possible_ends[topdowngen]
            #if topdowngen ==0:
            #    goal_position = (obstacle_coordinates[self.goalselector][0]+1, obstacle_coordinates[self.goalselector][1])
            #goal_position = possible_ends[topdowngen]
            #else:    
            #    goal_position = (obstacle_coordinates[self.goalselector][0]-1, obstacle_coordinates[self.goalselector][1])
    ...
    def _generate_obstacle_positions1(self):
        # Generating obstacles
        obstacle_coordinates = [(2, 0, 6, 3), (2, 5, 6, 7)]  # Define the combined obstacle coordinates
        num_obstacles = len(obstacle_coordinates)
        obstacle_positions = []

        for obstacle_coords in obstacle_coordinates:
            obstacle_row1, obstacle_col1, obstacle_row2, obstacle_col2 = obstacle_coords
            obstacle_size = (obstacle_row2 - obstacle_row1 + 1, obstacle_col2 - obstacle_col1 + 1)
            obstacle_positions.append(((obstacle_row1, obstacle_col1), obstacle_size))
