...
class DatasetNormalizer:
    ...
    def __init__(self, dataset, normalizer, path_lengths=None):
        dataset = flatten(dataset, path_lengths) #dict_keys(['actions', 'epiReward', 'next_observations', 'observations', 'rewards', 'skill', 'terminals', 'timeouts'])

        #indices_5 = np.where(arr == 5)[0]
        #indices_1 = np.where(arr == 1)[0]
        #self.startpo = 
