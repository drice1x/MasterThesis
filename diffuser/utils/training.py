...
class Trainer(object):
    ...
    def convert_batch_to_float(batch):
        new_batch = {}
        for key, value in batch.items():
            if isinstance(value, dict):
                new_dict = {}
                for sub_key, sub_value in value.items():
                    new_dict[sub_key] = convert_to_float(sub_value)
                new_batch[key] = new_dict
            else:
                new_batch[key] = convert_to_float(value)
        return new_batch
    ...
    def train(self, n_train_steps):
        ...
        for step in range(n_train_steps):
            for i in range(self.gradient_accumulate_every):
                ...
                #batch.trajectories = batch.trajectories.to(torch.float)
                # Convert tensors in conditions dictionary to torch.float
                #for key in batch.conditions:
                #    batch.conditions[key] = batch.conditions[key].to(torch.float)
                # Convert returns tensor to torch.float
                #batch.returns = batch.returns.to(torch.float)
                loss, infos = self.model.loss(*batch)
                #loss, infos = self.model.loss(batch.trajectories, batch.conditions, batch.returns)
                loss = loss / self.gradient_accumulate_every
                #print(type(loss))
                # Convert a tensor to Double (float64)
                loss = loss.double()  # Or tensor.to(torch.float64)
    ...
    def render_reference(self, batch_size=10):
        ...
        #self.renderer.composite(savepath, observations) #aaron: renderer entfernt
...
#            self.renderer.composite(savepath, observations)
    ...
    def inv_render_samples(self, batch_size=2, n_samples=2):
        ...
        for i in range(batch_size):
            ...
            savepath = os.path.join('images', f'sample-{i}.png')
            #self.renderer.composite(savepath, observations) ##aaron: rendern deaktiviert