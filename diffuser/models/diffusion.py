...
class GaussianInvDynDiffusion(nn.Module):
    ...
    def loss(self, x, cond, returns=None):
        if self.train_only_inv:
            ...
            batch_size = len(x)
            t = torch.randint(0, self.n_timesteps, (batch_size,), device=x.device).long()
            diffuse_loss, info = self.p_losses(x[:, :, self.action_dim:], cond, t, returns.to(torch.float64))
            # Calculating inv loss
            x_t = x[:, :-1, self.action_dim:]
            a_t = x[:, :-1, :self.action_dim]
            x_t_1 = x[:, 1:, self.action_dim:]
            ...
            if self.ar_inv:
                ...
                #x_comb_t = x_comb_t
                x_comb_t = x_comb_t.to(self.inv_model[0].weight.dtype) #to float64
                #hier kommt der fehler: erste zeile
                pred_a_t = self.inv_model(x_comb_t)
                #pred_a_t = pred_a_t.to(a_t.dtype)  # Convert pred_a_t to the same dtype as a_t
                inv_loss = F.mse_loss(pred_a_t, a_t)
